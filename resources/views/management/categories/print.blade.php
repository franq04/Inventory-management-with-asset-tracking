...
<head>
    ...
    <title>Categories Registry - Print</title>
    <style>
        @media print {
            @page {
                size: landscape;
...
<body>
    @php
        $recordsPerPage = 25;
        $chunks = $categories->chunk($recordsPerPage);
    @endphp
    ...
    @foreach($chunks as $chunkIndex => $chunk)
            ...
            <tbody>
                @foreach($chunk as $category)
                    <tr>
                        ...
                        <td style="text-align: center;">{{ $category->pqs_records_count ?? 0 }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        @if($loop->last)
            <div class="summary">
                <strong>Summary:</strong> Total Categories: {{ number_format($categories->count()) }} |
