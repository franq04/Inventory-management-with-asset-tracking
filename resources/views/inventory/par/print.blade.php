...
<head>
    ...
    <title>PAR Print - {{ $parRecord->par_no }}</title>
...
<body class="bg-white text-black p-8 text-sm font-sans">
    <div class="max-w-[11in] mx-auto border-2 border-black p-6 rounded-lg relative h-auto flex flex-col print-card" style="width: 10in; height: 7.5in;">@php $printDate = optional($parRecord->date_acquired) ? optional($parRecord->date_acquired)->format('M d, Y') : now()->format('M d, Y'); @endphp
        {{-- Header --}}
        <div class="text-center mb-6 relative">
            ...
            <h1 class="font-bold text-xl uppercase tracking-widest mb-1">Property Acknowledgment Receipt</h1>
        ...
        <div class="mb-4">
             <div class="flex items-center gap-2 w-1/3">
                <span class="font-bold whitespace-nowrap">PAR NO.:</span>
                <span class="border-b border-black flex-grow px-2 font-mono font-bold">{{ $parRecord->par_no }}</span>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-auto">
            @php
                ...
                $rawDescription = strip_tags((string) $parRecord->article_desc);
            ...
            <table class="w-full border-collapse border border-black mb-4 h-full">
                <thead>
                    <tr class="text-center font-bold text-xs bg-gray-100">
                        ...
                        <th class="border border-black p-2 w-40">PROPERTY NO.</th>
                        <th class="border border-black p-2 w-24">ESTIMATED USEFUL LIFE</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="text-center align-top">
                        <td class="border border-black p-2">{{ number_format($parRecord->quantity ?? 1) }}</td>
                        <td class="border border-black p-2">{{ $parRecord->unit ?? 'pc' }}</td>
                        <td class="border border-black p-2">{{ number_format($parRecord->unit_value ?? 0, 2) }}</td>
                        <td class="border border-black p-2">{{ number_format($parRecord->amount ?? 0, 2) }}</td>
                        <td class="border border-black p-2 text-left">
                            {{ $displayDescription }} // truncated for print, full text in PQS record
                            @if($parRecord->pqsRecord && $parRecord->pqsRecord->property_no)
                                <br><span class="text-xs">(SN: {{ $parRecord->pqsRecord->property_no }})</span>
                            @endif
                        </td>
                        <td class="border border-black p-2">{{ $parRecord->pqsRecord->property_no ?? 'N/A' }}</td>
                        <td class="border border-black p-2">{{ $parRecord->estimated_useful_life ?? 'N/A' }}</td>
                    </tr>
            ...
            <div class="w-1/2">
                ...
                <div class="mb-1">
                    <div class="border-b border-black h-8 flex items-end justify-center pb-1">
                        @if($parRecord->pqsRecord && $parRecord->pqsRecord->accountableOfficer)
                            <span class="font-bold text-xs uppercase">{{ $parRecord->pqsRecord->accountableOfficer->full_name }}</span>
                        @endif
                    </div>
                    <p class="text-xs mb-4">Signature over Printed Name</p>
                </div>
                ...
                <div class="mb-1">
                    <div class="border-b border-black h-4 flex items-end pb-0.5">
                        @if($parRecord->pqsRecord && $parRecord->pqsRecord->accountableOfficer && $parRecord->pqsRecord->accountableOfficer->position)
                            <span class="text-xs">{{ $parRecord->pqsRecord->accountableOfficer->position->position_name }}</span>
                        @endif
                    </div>
                    <p class="text-xs mb-6">Position/Office</p>
                </div>
